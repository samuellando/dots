{
    ...
    let 
      ...
      packagesFor = forAllSystems (system: with nixpkgsFor.${system}; [
          ...
          containerPackagesFor = forAllSystems (system: packagesFor.${system} ++ [
            nixpkgsFor.${system}.cacert
            nixpkgsFor.${system}.nix
          ]);
        in
        {
          pkgs = forAllSystems (system: nixpkgsFor.${system});  
          buildContainer = forAllSystems (system: 
            let
                pkgs = nixpkgsFor.${system};
                containerPackages = containerPackagesFor.${system};
            in
              {extraPackages ? [], extraShellHooks ? ""} : 
                  let 
                    dots = self;
                    shellHook = ''
                      ln -s /bin/zsh /bin/sh
